...
public class LancetHookTool {
    ...
    public void _loadConfig(boolean useCache, ApiConfig.LoadConfigCallback callback, Activity activity) {
        ...
        if (originUrl.isEmpty()) {
            ...
            history.add("https://mirror.ghproxy.com/raw.githubusercontent.com/gaotianliuyun/gao/master/0825.json");
            history.add("http://xhww.fun/小米/DEMO.json");
    ...
    protected void _onCreate(Bundle savedInstanceState) {
        ...
        if (o instanceof AboutDialog) {
            ...
            stringBuilder.append(String.format("版本：%s.%d for %s\n", BuildConfig.VERSION_NAME, BuildConfig.VERSION_CODE, arch));
            stringBuilder.append(String.format("打包时间：%s\n", BuildConfig.PACK_TIME));
            stringBuilder.append("本软件只提供聚合展示功能，所有资源来自网上, 软件不参与任何制作, 上传, 储存, 下载等内容. 软件仅供学习参考, 请于安装后24小时内删除。\n");
            stringBuilder.append("打包分发请保留出处\n");
            stringBuilder.append("https://github.com/CatVodTVOfficial/TVBoxOSC");
            ...
            if (window != null) {
                ...
                if (rootView != null && rootView.getChildCount() > 0) {
                    ...
                    if (child instanceof TextView) {
                        ((TextView) child).setText(stringBuilder);
                    }
                }
            }
        }
